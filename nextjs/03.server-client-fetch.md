...
**阅读时间预期**：本篇是系列最长的一篇（概念 + 对照 + 实战），建议预留 **4～6 小时**（含动手）。可拆成两次：第一次做到 §7 `lib/users`；第二次做 §10 搜索与 §12 详情。
...
![服务端组件 vs 客户端组件](../image/nextjs-03-server-client/01-server-client-compare.png)
...
拿不准时，按下面四问过滤——**任一条为「是」且涉及浏览器 API / 事件，就倾向 Client**：

| 问题 | 是 → 倾向 |
|------|-----------|
| 要用 `useState` / `useEffect` / `useRef` 吗？ | Client |
| 要 `onClick` / `onChange` / 拖拽吗？ | Client |
| 要读 `window` / `localStorage` / 地理位置吗？ | Client |
| 只是 `await fetch` 后 `map` 展示，无交互？ | Server |

边界情况：**列表项是 `Link` 跳转**——仍可在 Server 组件里写 `next/link`，不必 Client。  
**列表项是 `button` 选中高亮**——含 `onClick` 的列表组件要 Client，或父 Client 子展示。

![一分钟自检：代码该放哪](../image/nextjs-03-server-client/03-self-check.png)
...
![两种取数路径对照](../image/nextjs-03-server-client/02-data-flow.png)
...
### 11.3 try/catch 与 error.js 如何选

同一路由不要**又**在 page 里 `try/catch` **又**依赖 `error.js` 处理同一类错误——初学选一种即可：

| 你的目标 | 推荐 |
|----------|------|
| 先跑通、错误文案写死在 page 里 | `try/catch`（§5.2） |
| 想练「重试」按钮、统一错误 UI | `error.js` + `throw`（§11.2） |
| 部分字段失败、其余照常展示 | page 内 `try/catch` 更灵活 |

`error.js` 必须是 `'use client'`——它是**错误边界**，捕获子 Server Component 抛出的未处理错误。你把 `getUsers` 包在 try 里并 `return <p>出错了</p>` 时，错误**不会**冒泡到 `error.js`。
