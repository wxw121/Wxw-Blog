...
![资源命名六大铁律](image/rest-api-design/05-bento-grid-diagram-05.png)
...
```python
# 方案一：把动作建模为子资源（推荐）
POST /api/users/123/activate        # 激活——创建一个激活记录
POST /api/orders/456/approve        # 审批——创建一个审批记录
POST /api/newsletters/789/send      # 发送——创建一个发送记录

# 方案二：用 PATCH 修改状态字段（也很常见）
PATCH /api/users/123
body: {"status": "active"}
...
POST /api/approve-order/456

# 只要你的所有动作都能统一用「把动作名词化」的方式，
# 就已经比 90% 的 API 设计了。
```
...
![┬──────────┬─────────────┬](image/rest-api-design/06-comparison-matrix-diagram-06.png)
...
DELETE 也是幂等的——删除一个已经不存在的东西，结果还是「不存在」。
...
![┬](image/rest-api-design/08-comparison-matrix-diagram-08.png)
...
> 最核心的原则：**用 `data` 字段包裹数据**。这样前端可以统一用 `response.data` 取数据，而且能区分「data 是 null」（有意义的值）和「请求失败没有 data」两种情况。
...
// ❌ 避免
{
    "created_at": "2024-03-15",               // 太粗糙，没有时分秒
    "date": "15/03/2024 10:30",              // 格式不标准，有歧义
    "timestamp": 1710498600                   // Unix 时间戳，人类不可读
}
```
...
✅ URL 不变，严格符合 REST 理论
❌ 不好调试，不能直接在浏览器里试
❌ 很多 HTTP 代理会缓存时忽略 header
⚠️ 大多数团队在实践中还是选 URL 路径版本
```
...
![资源关系图](image/rest-api-design/12-comparison-matrix-资源关系图.png)
...
反模式 5: 暴露数据库主键
❌ GET /api/users/42  (用户知道自己排第 42 号)
✅ GET /api/users/xk9a2b  (用 UUID 或雪花 ID，防止被遍历)
...
![┬──────────────┬──────────────┬](image/rest-api-design/13-comparison-matrix-diagram-13.png)
