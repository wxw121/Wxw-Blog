...
直接在默认分支上改代码，等于**跳过审查、跳过 CI、跳过隔离**。

![为什么不直接 push master](../image/git-branch-strategy/02-bento-grid-diagram-02.png)

上图列出的风险可通过「feature 分支 + PR + 保护规则」规避。默认分支应只接收合并后的、可发布的代码。
...
Git Flow 由 Vincent Driessen 在 2010 年提出。读图时抓住五条长期分支的角色：**main**、**develop**、**feature**、**release**、**hotfix**。

![Git Flow 分支模型](../image/git-branch-strategy/03-comparison-matrix-Git-Flow-分支模型.png)

分支合并方向（feature→develop→release→main，hotfix→main+develop）是 Git Flow 的骨架；§3.2 用命令走一遍完整场景。
...
![GitHub Flow 工作流](../image/git-branch-strategy/04-linear-progression-GitHub-Flow.png)

与 Git Flow 相比，没有 develop/release 分支——适合**持续部署**、小团队、Web 产品。
...
TBD 把集成分支和发布分支都省掉了：短生命周期 feature 分支 + 频繁合并 + 自动化测试。
...
从开分支到合并删除，PR 是「代码进入主干的唯一通道」。读下图时对照每个检查点（CI、Review、冲突处理）。

![Pull Request 生命周期](../image/git-branch-strategy/06-comparison-matrix-Pull-Request-生命周期.png)

典型路径：`feature 分支` → 推送 → 开 PR → Code Review → CI → 合并 → 删分支。
...
理想 PR：**200–400 行**（含测试）；超过 **800 行**审查者容易跳过细节。大功能应拆成多个可独立合并的小 PR。

![PR 大小的黄金法则](../image/git-branch-strategy/07-bento-grid-diagram-07.png)

对照上图的拆分示例：把「重构订单系统」拆成接口提取、实现、迁移调用方、删旧代码、移除功能开关五步，每步 PR 可独立审查和回滚。
...
审查评论建议分四级：**blocking**（必须修）、**important**（应该修）、**suggestion**（建议）、**question**（讨论）。

![Code Review 评论速查](../image/git-branch-strategy/08-bento-grid-diagram-08.png)

对照上图：blocking 不超过 3 条——超过说明 PR 可能需要重新设计或拆分。
...
**Conventional Commits** 用固定格式写 commit message：`type(scope): subject`，可选 body 和 footer。

![Conventional Commits 提交信息结构](../image/git-branch-strategy/09-comparison-matrix-提交信息的结构.png)

常用 `type`：`feat` 新功能、`fix` 修 bug、`docs` 文档、`refactor` 重构、`chore` 杂项。
...
合并 feature 进 main 时有三种常见策略，区别在于**主干历史是否保留分叉**。

![Merge / Squash / Rebase 三种合并策略对比](../image/git-branch-strategy/10-bento-grid-diagram-10.png)

**Merge commit** 保留完整分支历史；**Squash** 压成一条干净提交；**Rebase** 改写历史使主干线性（共享分支慎用 force rebase）。
...
冲突时需要同时看**共同祖先（base）、当前分支（ours）、目标分支（theirs）** 三个版本。

![Git 三路合并 (Three-Way Merge) 示意](../image/git-branch-strategy/11-comparison-matrix-三路合并-Three-Way-Merge.png)

VS Code 合并编辑器会把三栏并排展示；复杂冲突仍建议和同事对一下业务语义。
...
速记：**固定发布周期** → Git Flow；**持续部署** → GitHub Flow；**大团队强 CI** → Trunk-Based。
