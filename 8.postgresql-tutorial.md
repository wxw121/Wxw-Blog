...
![┬──────────────────┬](image/postgresql/02-comparison-matrix-diagram-02.png)
...
![子查询:](image/postgresql/03-bento-grid-diagram-03.png)
...
![┬](image/postgresql/04-comparison-matrix-diagram-04.png)
...
![┬](image/postgresql/05-comparison-matrix-diagram-05.png)
...
**法则三：用 `TIMESTAMPTZ` 而不是 `TIMESTAMP`**

```sql
-- ❌ TIMESTAMP——不知道时区，冬令时夏令时切换时出问题
-- ✅ TIMESTAMPTZ——带时区，自动转换，永远不会错
...
**法则五：用 `COPY` 而不是 `INSERT` 做大批量导入**

```sql
-- ❌ 100 万条 INSERT——慢
INSERT INTO users VALUES (...), (...), ...;

-- ✅ COPY——比 INSERT 快 10 倍以上
...
**法则八：JSONB 适合「经常一起读取」的数据，不要把所有列都换成 JSONB**
...
-- MySQL 中 '' 在很多情况下等于 NULL——迁移过来时很容易踩坑
