# -*- coding: utf-8 -*-
"""Expand8 large unique blocks."""

EXPANSIONS8 = {
    "langchain-document-loader": """## 33. 工程深化（langchain）
...
统计：每周空文本率、乱码率、缺字段率周报，联动 149 bad case。""",
    "langchain-text-splitter": """## 33. 工程深化（langchain）
...
拒绝过小 chunk：少于 50 字与下一块合并，减噪声。""",
    "llamaindex-index-types": """## 33. 工程深化（llamaindex）
...
完成 148 条：demo + 映射 + 不双栈声明。""",
    "llamaindex-query-engine": """## 33. 工程深化（llamaindex）
...
生产：不默认 Query Engine 黑盒，要可审计链。""",
    "llamaindex-agent": """## 33. 工程深化（llamaindex）
...
面试：知道循环与风险，生产固定链优先。""",
    "haystack-pipeline": """## 33. 工程深化（haystack）
...
Generator 温度 0 利于 141 Faithfulness 回归。""",
    "pipeline-vs-framework": """## 33. 工程深化（pipeline）
...
下一读 136 Parser 协议落地。""",
    "pluggable-parser-splitter-embedder": """## 33. 工程深化（pluggable）
...
下游 137 Store Retriever Generator 对称。""",
    "langchain-vectorstore": """## 33. 工程深化（langchain）
