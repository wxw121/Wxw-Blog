...
# 实践 2: Feature Flag（功能开关）
# 功能没做完但又需要合并到 main？用功能开关隐藏未完成的部分
...
☑ Require status checks to pass before merging
  ...
  (在合并前必须先 rebase 到最新的 main)

# 3. 对话必须解决
☑ Require conversation resolution before merging
  (所有 PR 评论都标记为 resolved 后才能合并)
...
**错误五：合并方向搞反——把 main 合并到 feature 分支**

```bash
# ❌ 在 feature 分支上:
git merge main
# 这会创建一个无意义的 merge commit，让你的分支历史变脏
...
git reset --hard HEAD~1        # 从错误的分支删除

# 「我 force push 了，别人说代码丢了」
# 😱 看看 reflog——Git 记录了每一次 HEAD 移动
git reflog
